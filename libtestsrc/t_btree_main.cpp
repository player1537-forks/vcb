...
void RunQuery(int numFeatures, float* myHyp, bTree& tree) {
	...
	printf("  numFibers=%d, numPassed=%d)\n",tree.numRecords,tree.numPass);
	free(lb); free(ub);
	//View all results
...
int main(int argc, char** argv)
{
	...
	vcbDebugLevelSet(VCB_DEBUG_OFF);
	SmoothGeometry(patient);
	NormalizeFeaturesCohort(patient, 0.0, 1.0);
	vcbDebugLevelSet(VCB_DEBUG_WARNING);
	...
	printf("--All (306) should have passed\n\n");

	fiberHyp[2*LENGTH_HYP+2]=(float)0.45;
	strcpy(printFiberEq,GetFiberEquation(patient.numFeatures,fiberHyp));
	printf("  %s\n",printFiberEq);
	RunQuery(patient.numFeatures,fiberHyp,tree);
	printf("--297 should have passed\n\n");

	fiberHyp[2*LENGTH_HYP+3]=(float)0.50;
	strcpy(printFiberEq,GetFiberEquation(patient.numFeatures,fiberHyp));
	printf("  %s\n",printFiberEq);
	RunQuery(patient.numFeatures,fiberHyp,tree);
	printf("--220 should have passed\n\n");

	fiberHyp[(patient.numFeatures-1)*LENGTH_HYP+3]=(float)0.25;
	strcpy(printFiberEq,GetFiberEquation(patient.numFeatures,fiberHyp));
	printf("  %s\n",printFiberEq);
	RunQuery(patient.numFeatures,fiberHyp,tree);
	printf("--15 should have passed\n\n");

	fiberHyp[0*LENGTH_HYP+2]=(float)0.05;
	strcpy(printFiberEq,GetFiberEquation(patient.numFeatures,fiberHyp));
	printf("  %s\n",printFiberEq);
	RunQuery(patient.numFeatures,fiberHyp,tree);
	printf("--None (0) should have passed\n\n");

	//Cleanup
	free(fiberHyp);
	free(clusterHyp);
	for(i=0;i<patient.numFibers;i++) {
		delete [] patient.fiber[i].vertexArray;
		free(patient.fiber[i].smoothedVertexArray);
		free(patient.fiber[i].tangentVertexArray);
		delete [] patient.fiber[i].featureArray;
