...
//Make launch command
    ...
    strncat(launchcmd, "mcube 0", strlen(".xml mcube 0"));
...
void creategui(void) {
  ...
  timestep_spinner->set_int_limits(1, numoftsteps, GLUI_LIMIT_WRAP);

  isoval_spinner = glui->add_spinner("Isovalue", GLUI_SPINNER_FLOAT, &isoval, ISOVAL_ID, control_cb);

  isoval_spinner->set_float_limits(0.0, 100.0, GLUI_LIMIT_WRAP);
