...
#else
	/* vol data formats are indexed by [(k*ydim+j)*xdim+i] */
...
#define _id3(x,y,z, dsize,i,na) ((((x)*dsize[1] + (y)) * dsize[2] + (z))*(na) + (i))
	...
	pool = (unsigned char *) malloc((sz[0]-1)*(sz[1]-1)*(sz[2]-1)*5);
	poolcnt = 0;
	for (i = 0; i < sz[0]-1; i ++)
		for (j = 0; j <sz[1]-1; j ++)
			for (k = 0; k <sz[2]-1; k ++) {
				...
				if (cellmax > 0) {
					pool[poolcnt * 5    ] = cellmax;
					pool[poolcnt * 5 + 1] = cellmin;
					pool[poolcnt * 5 + 2] = (unsigned char)i;
					pool[poolcnt * 5 + 3] = (unsigned char)j;
					pool[poolcnt * 5 + 4] = (unsigned char)k;

					poolcnt ++;
				}
			}

	root = vcbMntreeBuildub(32, 3, nattribs, poolcnt, 5, pool, nattribs, \
	                     sparseKeyFunc,  \
						 compareKeyFunc, \
						 decompKeyFunc,  \
						 boundKeyFunc);
	...
	printf("mntree has %d items, v.s. poolcnt %d\n", vcbMntreeStat(root),poolcnt);
...
void getmesh(void)
{
	int lb[2], ub[2], ncells, i, yzslice;
	short * cellids;
	void ** results;
	unsigned char * cids;
	clock_t time1, time2;
	...
	lb[0] = (int)isoval; ub[0] = 255;
	lb[1] = 0;           ub[1] = (int)isoval;

	ncells = vcbMntreeQueryub(root, nattribs, lb, ub, &results);
	...
	yzslice = sz[1] * sz[2];
	for (i = 0; i < ncells; i ++) {
		cids = (unsigned char *)(results[i]);
		cellids[i*3  ] = (short)cids[2];
		cellids[i*3+1] = (short)cids[3];
		cellids[i*3+2] = (short)cids[4];
	}
	free(results);
